...
def import_f8_reference_transcript(db: Session = Depends(get_db)):
    """
    Import the exact wild-type/reference F8 transcript used in the project report.

    Reference accession:
        NM_000132.4

    This is stronger than the broad F8 query because it retrieves the specific
    RefSeq mRNA record for Homo sapiens coagulation factor VIII (F8),
    transcript variant 1.

    Output:
        JSON with accession, imported term name, and sequence length.

    Diagnostic checkpoints:
        - status == success
        - accession == NM_000132.4
        - sequence_length > previous partial F8 record length
        - MySQL terms table has disease_class = 'Hemophilia A / F8 reference transcript'
    """
    accession = "NM_000132.4"
    ...
    try:
        ...
        if fasta_res.status_code != 200 or not fasta_res.text.strip():
            return {
                "status": "error",
                "message": f"Could not retrieve reference transcript {accession} from NCBI."
            }

        fasta_text = fasta_res.text
        lines = fasta_text.splitlines()
        header = lines[0].replace(">", "").strip() if lines else accession
        clean_sequence = "".join(
            line.strip()
            for line in lines
            if not line.startswith(">")
        ...
        is_valid, validation_result = is_valid_dna(clean_sequence)
        if not is_valid:
            return {
                "status": "error",
        ...
        if existing:
            return {
                ...
                "term_id": existing.id,
                "term": existing.term,
                "sequence_length": len(existing.fasta_seq or "")
            }

        new_term = models.Term(
            term=header[:255],
            trans="F8 reference transcript NM_000132.4",
            defe=(
                f"Exact NCBI RefSeq reference transcript accession: {accession}. "
                "Used as the wild-type F8 cDNA/mRNA reference for the Hemophilia A project."
            ),
            fasta_seq=clean_sequence,
            sequence=clean_sequence,
